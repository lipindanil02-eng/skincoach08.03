...
VIS_FB = [m.strip() for m in os.getenv("VISION_FALLBACKS", "google/gemini-2.0-flash-001").split(",") if m.strip()]
TXT_FB = [m.strip() for m in os.getenv("TEXT_FALLBACKS", "google/gemini-2.0-flash-001").split(",") if m.strip()]
